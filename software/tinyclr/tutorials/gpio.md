...
## LowLevel Access
**For advanced users only**. To use this here is the ways to `SetAlternate()` function and also to control `ReservePin()`. This requires you to reference the orignal chip manufactures datasheet to see which pins offer alternative functions. This unsupported feature requires an understanding of chip registers and how they work.  The chip internally may support certain peripheral on more that one pin. We select default ones and those are the one that everyone must use and they are the ones listed in the official pinout. However, advanced users have the option to move the peripheral, or one of the pins on that peripheral, to an alternate pin.
